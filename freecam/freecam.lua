...
local function freecamFrame()
	...
	if options.smoothMovement then
		...
		if
			(getKeyState(options.key_backward) or getPedControlState(options.key_backward_veh))
...
local function freecamMouse(cX, cY, aX, aY)
	...
	rotX = rotX + aX * options.mouseSensitivity * 0.01745329252
	rotY = rotY - aY * options.mouseSensitivity * 0.01745329252
